...
/* Construct the full path of the module shared library we’ll try to load. */
  ...
  module_generate = (void (*) (int)) dlsym (handle, "module_generate"); /* Make sure the symbol was found. */
...
/* Allocate and initialize a server_module object. */
  module = (struct server_module*) xmalloc (sizeof (struct server_module)); module->handle = handle;
