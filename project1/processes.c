...
static int get_uid_gid (pid_t pid, uid_t* uid, gid_t* gid)
{
  ...
  snprintf (dir_name, sizeof (dir_name), "/proc/%d", (int) pid); /* Obtain information about the directory. */
...
/* Return the name of group GID.
The return value is a buffer that the caller must allocate with free.
GID must be a valid group ID. */
...
The return value is a newly allocated buffer which the caller must deallocate with free. */
static char* get_program_name (pid_t pid) {
 ...
 /* Read the contents. */
  ...
  if (rval <= 0)
    ...
    status_info[rval] = '\0';
    /* The program name is the second element of the file contents and is surrounded by parentheses.
    ...
    strncpy (result, open_paren + 1, close_paren - open_paren - 1); /* strncpy doesn't NUL-terminate the result, so do it here. */ result[close_paren - open_paren - 1] = '\0';
    /* All done. */
    return result;
}
...
void module_generate (int fd) {
  size_t i;
  DIR* proc_listing;
  /* Set up an iovec array. We'll fill this with buffers that'll be part of our output,
  growing it dynamically as necessary. */
  ...
  while (1) {
    ...
    /* We've hit the end of the listing. */
      break;
      /* If this entry is not composed purely of digits,
      it's not a process directory, so skip it. */
    ...
    if (process_info == NULL)
      ...
      The process may have vanished while we were looking at it.
      Use a placeholder row instead. */
      process_info = "<tr><td colspan=\"5\">ERROR</td></tr>";
      /* Make sure the iovec array is long enough to hold this buffer
      (plus one more because we'll add an extra element when we're done listing processes). If not, grow it to twice its current size. */
  ...
  /* Deallocate the buffers we created. The first and last are static and should not be deallocated. */
