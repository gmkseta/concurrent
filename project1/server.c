...
static void handle_get (int connection_fd, const char* page)
{
  ...
  if (module == NULL) {
    ...
    snprintf (response, sizeof (response), not_found_response_template, page); /* Send it to the client. */
...
static void handle_connection (int connection_fd)
{
  ...
  if (bytes_read > 0) {
    ...
    For this HTTP implementation, we don't care about it.
    However, we need to read any data the client tries to send.
    Keep on reading data until we get to the end of the header, which is delimited by a blank line.
    HTTP specifies CR/LF as the line delimiter. */
    while (strstr (buffer, "\r\n\r\n") == NULL)
    ...
    if (strcmp (protocol, "HTTP/1.0") && strcmp (protocol, "HTTP/1.1")) { /* We don't understand this protocol. Report a bad response. */ write (connection_fd, bad_request_response,
...
void server_run (struct in_addr local_address, uint16_t port)
{
  ...
  memset (&socket_address, 0, sizeof (socket_address)); socket_address.sin_family = AF_INET;
  ...
  while (1){
    ...
    if (connection == -1) {
      ...
      if (errno == EINTR)/* The call was interrupted by a signal. Try again. */
        continue;
